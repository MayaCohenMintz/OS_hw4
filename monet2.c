...
void enqueue(void* data) {
    ...
    if (queue.waiting > 0) {
        ...
        while (curr && !curr->is_thread) {
            curr = curr->next;
        }
        if (curr && curr->is_thread) {
...
void* dequeue(void) {
    ...
    while (result == NULL) {
        if (queue.front && !queue.front->is_thread) {
            ...
            // After waking up, remove the thread node
            if (queue.front == thread_node) {
                queue.front = queue.front->next;
            } else {
                Node* prev = queue.front;
                while (prev && prev->next != thread_node) {
                    prev = prev->next;
                }
                if (prev) prev->next = thread_node->next;
            }
            if (queue.rear == thread_node) {
                queue.rear = NULL;
            }
